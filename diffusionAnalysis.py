    ...
    for col in columns:
        ...
        for dir in dirLists:
            ...
            for file in fileLists:
                ...
                D_errorbar = (Dd_interval[1] - Dd) / dim
                df_current = pd.DataFrame([{
                    'foldername': folderName,
                    'molecule': molecule,
                    'concentration': int(moleculeConcentration[molecule]),
            ...
            if (len(moleculeConcentration) > 1):
                ...
                D_mix = 0
                Derrorbar_mix = 0 # errorbar
                for i in range(len(df_temp)):
                    D_mix += df_temp.iloc[i]["concentration"] * df_temp.iloc[i]["diffusion_coeff"]
                    Derrorbar_mix += df_temp.iloc[i]["concentration"] * df_temp.iloc[i]["diffusion_errorbar"]
                D_mix /= 100
                Derrorbar_mix /= 100
                df_current = pd.DataFrame([{
                    'foldername' : folderName,
                    ...
                    'diffusion_errorbar': Derrorbar_mix
                }])
                dfFinal = dfFinal.append(df_current, ignore_index = True)
    return dfFinal
